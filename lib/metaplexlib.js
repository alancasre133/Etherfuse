...
metaplexlib.createNFT = async (nftData) => {
    try {
        ...
        const metadata = {
            name: "Studevs",
            description: `Un NFT De una comunidad de estudiantes de la UASLP`,
            image: imageUrl,
            symbol: "SD",
            collection: {
                name: "Studevs tulumboys",
                family: "Studevs",
            },
        };
            ...
            const { response: nftCreated } = await metaplex.nfts().create(
                {
                    uri: data.uri,
                    name: metadata.name,
                    sellerFeeBasisPoints: 500, // porcentaje de comision para el creador por venta de NFT en base a 100 100 puntos equivale a 1%
